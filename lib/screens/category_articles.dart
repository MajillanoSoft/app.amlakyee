...
class ColorArticles extends StatefulWidget {
  final int id;
  final String name;
  ColorArticles(this.id, this.name, {Key key}) : super(key: key);
  @override
  _ColorArticlesState createState() => _ColorArticlesState();
}

class _ColorArticlesState extends State<ColorArticles> {

  Future<List> fetchCarsbycolor() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/cars?_embed&color[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
            future: fetchCarsbycolor(),
            builder: (context, colorsnapshot) {
              if (colorsnapshot.hasData) {
                if(colorsnapshot.data.length == 0) {
                ...
                return ListView.separated(
                  itemCount: colorsnapshot.data.length,
                  padding: EdgeInsets.symmetric(vertical: 15, horizontal: 0),
                  itemBuilder: (BuildContext context, int index) {
                    Map wpData = colorsnapshot.data[index];
                    var ImgUrl =
