...
class adArticles extends StatefulWidget {
  final int id;
  final String name;
  adArticles(this.id, this.name, {Key key}) : super(key: key);
  @override
  _adArticlesState createState() => _adArticlesState();
}

class _adArticlesState extends State<adArticles> {


  Future<List> fetchEstatebyad() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/real_estate?_embed&sale_rent[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
            future: fetchEstatebyad(),
            builder: (context, adsnapshot) {
              if (adsnapshot.hasData) {
                if(adsnapshot.data.length == 0) {
                ...
                return ListView.separated(
                  itemCount: adsnapshot.data.length,
                  padding: EdgeInsets.symmetric(vertical: 15, horizontal: 0),
                  itemBuilder: (BuildContext context, int index) {
                    Map wpData = adsnapshot.data[index];
                    var ImgUrl =
