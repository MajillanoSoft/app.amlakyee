...
class ByEstatesPrice extends StatefulWidget {
  final int priceval;
  final String name;
  ByEstatesPrice(this.priceval, this.name, {Key key}) : super(key: key);
  @override
  _ByEstatesPriceState createState() => _ByEstatesPriceState();
}

class _ByEstatesPriceState extends State<ByEstatesPrice> {


  Future<List> fetchEstatebyprice() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/real_estate?filter[meta_key]=_diwp_text_field&filter[meta_compare]=<&filter[meta_value]=${widget.priceval.toString()}?_fields=link,title,featured_media,_links,_embedded&_embed&per_page=100", headers: {'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
            future: fetchEstatebyprice(),
            builder: (context, pricesnapshot) {
              if(pricesnapshot.data.length == 0) {
              ...
              if (pricesnapshot.hasData) {
                return ListView.separated(
                  itemCount: pricesnapshot.data.length,
                  padding: EdgeInsets.symmetric(vertical: 15, horizontal: 0),
                  itemBuilder: (BuildContext context, int index) {
                    Map wpData = pricesnapshot.data[index];
                    var ImgUrl =
                                    ...
                                    Column(
                                      ...
                                      children: <Widget>[
                                        Container(
                                          ...
                                          child: Text(
                                            wpData["meta_box"]["_diwp_text_field"] != "" ? " ${wpData["meta_box"]["_diwp_text_field"]} مليون دينار"
                                                : "" ,
                                            style: TextStyle(
                                              color: Colors.white,
                                              fontSize: 13,
