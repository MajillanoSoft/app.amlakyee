...
class CarsLocationArticles extends StatefulWidget {
  final int id;
  final String name;
  CarsLocationArticles(this.id, this.name, {Key key}) : super(key: key);
  @override
  _CarsLocationArticlesState createState() => _CarsLocationArticlesState();
}

class _CarsLocationArticlesState extends State<CarsLocationArticles> {

  Future<List> fetchCarsbylocation() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/cars?_embed&locationCars[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
