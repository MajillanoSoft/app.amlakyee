...
class CarsTypeArticles extends StatefulWidget {
  final int id;
  final String name;
  CarsTypeArticles(this.id, this.name, {Key key}) : super(key: key);
  @override
  _CarsTypeArticlesState createState() => _CarsTypeArticlesState();
}

class _CarsTypeArticlesState extends State<CarsTypeArticles> {


  Future<List> fetchCarsbytype() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/cars?_embed&carType[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
            future: fetchCarsbytype(),
            builder: (context, cartypesnapshot) {
              if (cartypesnapshot.hasData) {
                if(cartypesnapshot.data.length == 0) {
                ...
                return ListView.separated(
                  itemCount: cartypesnapshot.data.length,
                  padding: EdgeInsets.symmetric(vertical: 15, horizontal: 0),
                  itemBuilder: (BuildContext context, int index) {
                    Map wpData = cartypesnapshot.data[index];
                    var ImgUrl =
