...
class ByCarsDate extends StatefulWidget {
  final int id;
  final String name;
  ByCarsDate(this.id, this.name, {Key key}) : super(key: key);
  @override
  _ByCarsDateState createState() => _ByCarsDateState();
}

class _ByCarsDateState extends State<ByCarsDate> {

  Future<List> fetchCarsbydate() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/cars?_embed&date_cars[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
