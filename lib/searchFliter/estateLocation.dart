...
class estateLocationArticles extends StatefulWidget {
  final int id;
  final String name;
  estateLocationArticles(this.id, this.name, {Key key}) : super(key: key);
  @override
  _estateLocationArticlesState createState() => _estateLocationArticlesState();
}

class _estateLocationArticlesState extends State<estateLocationArticles> {


  Future<List> fetchEstatebylocation() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/real_estate?_embed&location[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
            future: fetchEstatebylocation(),
            builder: (context, locationsnapshot) {
              if (locationsnapshot.hasData) {
                if(locationsnapshot.data.length == 0) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    Map wpData = locationsnapshot.data[index];
                    var ImgUrl =
