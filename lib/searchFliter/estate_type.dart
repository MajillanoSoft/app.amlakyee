...
class EstateTypeArticles extends StatefulWidget {
  final int id;
  final String name;
  EstateTypeArticles(this.id, this.name, {Key key}) : super(key: key);
  @override
  _EstateTypeArticlesState createState() => _EstateTypeArticlesState();
}

class _EstateTypeArticlesState extends State<EstateTypeArticles> {


  Future<List> fetchEstatebytype() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/real_estate?_embed&estateType[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
            future: fetchEstatebytype(),
            builder: (context, typesnapshot) {
              if (typesnapshot.hasData) {
                if(typesnapshot.data.length == 0) {
                ...
                return ListView.separated(
                  itemCount: typesnapshot.data.length,
                  padding: EdgeInsets.symmetric(vertical: 15, horizontal: 0),
                  itemBuilder: (BuildContext context, int index) {
                    Map wpData = typesnapshot.data[index];
                    var ImgUrl =
