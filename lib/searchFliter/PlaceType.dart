...
class landTypeArticles extends StatefulWidget {
  final int id;
  final String name;
  landTypeArticles(this.id, this.name, {Key key}) : super(key: key);
  @override
  _landTypeArticlesState createState() => _landTypeArticlesState();
}

class _landTypeArticlesState extends State<landTypeArticles> {


  Future<List> fetchEstatebyland() async {
    final response =
    await http.get(
        "$Webaite_URL/wp-json/wp/v2/real_estate?_embed&land_type[]=${widget.id}&per_page=100", headers: {'Accept': 'application/json'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder(
            future: fetchEstatebyland(),
            builder: (context, landsnapshot) {
              if (landsnapshot.hasData) {
                if(landsnapshot.data.length == 0) {
                ...
                return ListView.separated(
                  itemCount: landsnapshot.data.length,
                  padding: EdgeInsets.symmetric(vertical: 15, horizontal: 0),
                  itemBuilder: (BuildContext context, int index) {
                    Map wpData = landsnapshot.data[index];
                    var ImgUrl =
